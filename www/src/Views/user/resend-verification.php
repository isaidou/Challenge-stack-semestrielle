<?php View::header(true, "Renvoyer la vérification") ?>
...
<form action="<?= URLROOT ?>/user/resend-verification" method='POST' id='page-form' class='mt-3'>
    <h3 class='text-center mb-5'>Renvoyer la vérification</h3>
    <div class="form-group mb-3">
        <label for="email" class='d-block pb-2'>Entrez l'email: </label>
        <input type="text" class="form-control <?= !empty($data['error']) ?  'is-invalid' : '' ?>" name='email' placeholder="Adresse e-mail" value="<?= ht($data['default_mail']) ?>">
